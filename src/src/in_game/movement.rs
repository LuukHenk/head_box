...
pub fn handle_player_enemy_collision(
    ...
    for enemy_transform in enemies_query.iter() {
        ...
        if let Some(collision) = collision {
            player_movement = apply_collision_pushback(collision, player_movement, STRONG_COLLISION_PUSHBACK);
            player_health.0 -= 1.;
...
fn apply_collision_pushback(collision: Collision, mut movement: Mut<Movement>, pushback_strength: f32) -> Mut<Movement> {
    match collision {
